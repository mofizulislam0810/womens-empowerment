...
if(isset($_POST['submit'])){
    ...
    if($type == ''){
        ...
        echo "<script>window.open('tutorialform.php','_self')</script>";
        exit();
    }
    move_uploaded_file($image_tmp,$local_image.$post_image);
    $insert_query = "insert into tutorial (date,author,title,images,content,type) values ('$date','$author_name','$title','$post_image','$content','$type')";
    if(mysqli_query($con,$insert_query)){
    echo "<script>alert('Tutorial published successfuly')</script>";
    echo "<script>window.open('tutorialform.php','_self')</script>";
        }
    }
?>
